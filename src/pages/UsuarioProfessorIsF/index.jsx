...
const UsuarioProfessorIsF = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        const proefResponse = await api.get("/isf_teacher/my_proeficiency", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        console.log("JSON Completo:", proefResponse);

        // Busca instituição atual
        const institutionResponse = await api.get(
          "/isf_teacher/current_institution",
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        setProeficiencies(proefResponse.data?.data || []);
        console.log("Proeficiências Recuperadas:", proefResponse.data.proeficiencies);


        // Valida se a instituição existe
        const institutionId = institutionResponse.data?.data?.idInstituicao;
        ...
        if (institutionId) {
          ...
          setInstitutionData({
            nome: institutionData.nome || "Nenhuma instituição vinculada",
            comprovante: institutionResponse.data?.data?.comprovante || null,
          });
        } else {
          console.warn("Instituição não encontrada no registro atual.");
          setInstitutionData({
            nome: "Nenhuma instituição vinculada",
            comprovante: null,
          });
        }
      } catch (error) {
        ...
        // Define estados seguros para evitar loops
        setUserData(null);
        setProeficiencies([]);
        setInstitutionData({
          nome: "Nenhuma instituição vinculada",
  ...
  const handleEditClick = () => {
    navigate("/usuario-professorisf-edicao", {
      state: {
        userData,
        proeficiencies,
        institutionData,
                        ...
                        proeficiencies.length > 0 ? (
                          proeficiencies.map((proef, index) => (
                            ...
                            >
                              ...
                              <Input value={proef.idioma || "N/A"} isReadOnly />

                              <Text fontWeight="bold" mt="2">
                                Nível
                              </Text>
                              <Input value={proef.nivel || "N/A"} isReadOnly />
                              ...
                              {proef.comprovante ? (
                                <Button
                                  ...
                                  variant="outline"
                                  onClick={() =>
                                    alert(`Comprovante: ${proef.comprovante}`)
...
export default UsuarioProfessorIsF;
